...
Tabs = {
    ...
    ["Admin"]    = wrapTab(Window:AddTab("Admin")),
...
-- TAB ADMIN
-- ==========================================
Tabs.Admin:AddSection("Admin thật 100% (Real Admin 100%)")
local _kamuiOpened = false
local _kamuiStatus = Tabs.Admin:AddParagraph({
    ["Title"] = "Kamui Status: ❌ Chưa mở",
    ["Description"] = "Kamui Status: ❌ Locked | Bấm nút bên dưới để kích hoạt"
})
Tabs.Admin:AddButton({
    ["Title"] = "🌀 Kamui (Bấm 1 lần)",
    ["Description"] = "🌀 Kamui (Press Once) | Kích hoạt Admin Kamui",
    ["Callback"] = function()
        if not _kamuiOpened then
            _kamuiOpened = true
            pcall(function()
                loadstring(game:HttpGet("https://raw.githubusercontent.com/WhiteX1208/Scripts/refs/heads/main/AdminKamui.luau"))()
            end)
            pcall(function()
                _kamuiStatus:SetText("Kamui Status: ✅ Đã mở")
            end)
            Library:Notify({
                ["Title"] = "LuanOri Hub",
    ["Description"] = "LuanOri Hub | 🌀 Kamui đã được kích hoạt!",
                ["Duration"] = 3
            })
        else
            Library:Notify({
                ["Title"] = "LuanOri Hub",
    ["Description"] = "LuanOri Hub | ⚠️ Kamui đã mở rồi!",
                ["Duration"] = 2
            })
        end
    end
...
Tabs.Admin:AddSection("☄️ Meteor Rain")
local _throneStatus = Tabs.Admin:AddParagraph({
    ["Title"] = "Royal Throne: ⏳ Đang kiểm tra...",
    ["Description"] = "Royal Throne: ⏳ Checking... | Cần có Royal Throne mới dùng được"
})
local _meteorTimer = Tabs.Admin:AddParagraph({
    ["Title"] = "⏱️ Chu kỳ: ~3-4 giờ/lần",
    ["Description"] = "⏱️ Cycle: ~3-4 hours each | Thời gian trong server hiện tại: đang tính..."
})
local _meteorNextLabel = Tabs.Admin:AddParagraph({
    ["Title"] = "🕐 Server time: --:--:--",
    ["Description"] = "Khi Royal Throne xuất hiện = sự kiện đang diễn ra"
})

-- Kiem tra Royal Throne + dem thoi gian server
local _meteorStartTick = tick()
task.spawn(function()
    pcall(function()
        local lastStatus = ""
        local _eventStartTime = nil
        while true do
            task.wait(1)

            -- Kiem tra Royal Throne
            local chair = workspace:FindFirstChild("LuckymaxerChair")
            local exists = chair and chair:FindFirstChild("Royal Throne")
            local status = exists and "✅ Có (Sự kiện đang diễn ra!)" or "❌ Không có"
            if status ~= lastStatus then
                pcall(function() _throneStatus:SetText("Royal Throne: " .. status) end)
                if exists and not _eventStartTime then
                    _eventStartTime = tick()
            ...
            -- Dem thoi gian server (tu khi bat script)
            local elapsed = tick() - _meteorStartTick
            local h = math.floor(elapsed / 3600)
            local m = math.floor((elapsed % 3600) / 60)
            local s = math.floor(elapsed % 60)
            local timeStr = string.format("%02d:%02d:%02d", h, m, s)
            pcall(function()
                _meteorTimer:SetText("⏱️ Server uptime: " .. timeStr .. " | Chu kỳ: ~3-4h")
            end)

            -- Hien thi trang thai su kien
            if exists and _eventStartTime then
                local dur = tick() - _eventStartTime
                local dm = math.floor(dur / 60)
                local ds = math.floor(dur % 60)
                pcall(function()
                    _meteorNextLabel:SetText("🔥 SỰ KIỆN ĐANG DIỄN RA! (" .. dm .. "p " .. ds .. "s)")
                end)
            else
                -- Uoc tinh lan tiep (moi 3.5h = 12600s)
                local cycle = 12600
                local timeInCycle = elapsed % cycle
                local remaining = cycle - timeInCycle
                local rh = math.floor(remaining / 3600)
                local rm = math.floor((remaining % 3600) / 60)
                local rs = math.floor(remaining % 60)
                pcall(function()
                    _meteorNextLabel:SetText(string.format("🕐 Ước tính lần tiếp: ~%02dh %02dm %02ds", rh, rm, rs))
                end)
            end
        end
    end)
...
Tabs.Admin:AddButton({
    ["Title"] = "☄️ Meteor Rain Attack",
    ["Description"] = "Cần có Royal Throne trong workspace",
    ["Callback"] = function()
        local Players = game:GetService("Players")
        local player = Players.LocalPlayer
        local backpack = player:WaitForChild("Backpack")

        -- Kiem tra Royal Throne truoc
        local chair = workspace:FindFirstChild("LuckymaxerChair")
        if not chair or not chair:FindFirstChild("Royal Throne") then
            Library:Notify({
                ["Title"] = "LuanOri Hub",
    ["Description"] = "LuanOri Hub | ❌ Không tìm thấy Royal Throne!",
                ["Duration"] = 3
            })
            return
        end
        ...
        Library:Notify({
            ["Title"] = "LuanOri Hub",
    ["Description"] = "LuanOri Hub | ☄️ Meteor Rain đã được thêm vào túi! Equip và bấm để dùng.",
            ["Duration"] = 4
        })
    end
})
...
Tabs.Info:AddSection("Thông Tin (Information)")
Tabs.Info:AddButton({
    ["Title"] = "LuanOri Hub Script | Community",
    ["Description"] = "LuanOri Hub Script | Community | Discord",
    ["Callback"] = function()
        setclipboard(tostring("https://discord.gg/LuanOri"))
    end
})
Tabs.Info:AddButton({
    ["Title"] = "TikTok",
    ["Description"] = "LuanOri Cheats",
    ["Callback"] = function()
        setclipboard(tostring("https://tiktok.com/@LuanOri04"))
    end
})
Tabs.Info:AddButton({
    ["Title"] = "Shiina Mahiru ",
    ...
    ["Callback"] = function()
        setclipboard(tostring("https://t.me/ShiinaTenju"))
    end
})
Tabs.Info:AddParagraph({
    ["Title"] = "Phát triển Monster",
    ["Description"] = "Developed by LuanOri | Kỹ Năng: Dell"
})
local v252 = nil
...
Tabs.Info:AddParagraph({
    ...
    ["Description"] = "Latest Update | update sự kiện admin,auto egg event"
})
Tabs.Info:AddParagraph({
    ["Title"] = "Các Client Android và Ios Hợ Trỡ",
    ["Description"] = "Supported Android & iOS Clients | Tất Cả Client Android và Ios "
})
Tabs.Info:AddParagraph({
    ["Title"] = "Các Client Pc Hợ Trỡ",
    ["Description"] = "Supported PC Clients | Tất Cả Client Pc"
})
...
Tabs.Main:AddSection("Cày Cấp (Auto Farm Level)")
...
Tabs.Main:AddSection("Chiến Đấu (Combat)")
...
Tabs.Main:AddToggle("ToggleHakiFortress", {
    ...
    ["Description"] = "Enable Fortress Color Haki",
    ["Default"] = false
...
Tabs.Main:AddToggle("ToggleCollectBerry", {
    ...
    ["Description"] = "Fly To Berry+Hop Area",
    ["Default"] = false
...
Tabs.Main:AddSection("Thông Thạo (Mastery)")
...
Tabs.Main:AddToggle("ToggleMasteryFruit", {
    ["Title"] = "Cày Trái",
    ["Description"] = "Farm Fruit Mastery",
    ["Default"] = false
...
if Sea3 then
    Tabs.Main:AddSection("Tư Lệnh Bánh (Cake Commander)")
    ...
    ["Description"] = "Farm Cake Commander",
        ["Default"] = false
    ...
    ["Description"] = "Summon Cake Commander",
        ["Default"] = true
...
Tabs.Main:AddSection("Trùm (Boss)")
if Sea1 then
    tableBoss = {
        "The Gorilla King",
        "Bobby",
...
Tabs.Main:AddSection("Nguyên Liậu (Materials)")
if Sea1 then
    MaterialList = {
        "Scrap Metal",
        "Leather",
...
-- =====================================
-- 🥚 AUTO COLLECT EGG [BETA] - ĐẦU TAB SỰ KIỆN
-- =====================================
...
if World1 then
    IslandPositions = {
        ...
        CFrame.new(-4914.8212890625, 50.963626861572, 4281.0278320813),
...
Tabs.Sea:AddDropdown("DropEggMethod", {
    ["Title"] = "Select Method Egg [BETA]",
    ["Values"] = {"Normal", "Chest", "Island"},
    ["Default"] = "Normal"
}):OnChanged(function(Value)
    _G.ChooseEgg = Value
end)

Tabs.Sea:AddToggle("ToggleAutoEgg", {
...
spawn(function()
    ...
    local isFlying = false -- KHOA LOOP KHI DANG BAY

    while wait(1) do
        if _G.AutoCollectEgg then
            -- NEU DANG BAY THI KHONG LAM GI HET - tranh doi huong
                ...
                if not hasEgg then
                    ...
                    elseif _G.ChooseEgg == "Island" then
                        ...
                        if #IslandPositions > 0 and (tick() - lastIslandTP) > 2 then
                            isFlying = true -- KHOA LOOP TRUOC KHI BAY
                            ...
                            pcall(function()
                                if root then
                                    dist = (dest.Position - root.Position).Magnitude
                                end
                            end)
                            task.wait(math.clamp(dist / 300, 2, 15))
                            isFlying = false -- MO KHOA SAU KHI BAY XONG
                        end
...
if Sea3 then
    ...
    ["Description"] = "Kitsune Island Status",
    })
    ...
    Tabs.Sea:AddToggle("ToggleCollectAzure", {
        ["Title"] = "Nhặt Linh Hồn",
    ["Description"] = "Collect Azure Embers",
        ["Default"] = false
    ...
    Tabs.Sea:AddButton({
        ["Title"] = "Đổi Linh Hồn",
    ["Description"] = "Exchange Soul",
        ["Callback"] = function()
            game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RF/KitsuneStatuePray"):InvokeServer()
        end
    })
    Tabs.Sea:AddSection("Biển (Sea)")
    ...
    ["Description"] = "Sail To Tiki Island",
        ["Default"] = false
    ...
    ["Description"] = "Sail To Hydra Island",
        ["Default"] = false
    ...
    ["Description"] = "Fly To Boat | Duy Nhất Thuyền Bạn Mua \225\187\158 Chợ Chồn",
        ["Callback"] = function()
			-- upvalues: (ref) vu596
            vu596()
        end
...
if Sea3 then
    Tabs.Sea:AddSection("Đảo Bí Ẩn (Mystery Island)")
...
Tabs.Sea:AddToggle("ToggleTweenGear", {
    ...
    ["Description"] = "Fly To Gear Island",
    ["Default"] = false
...
Tabs.Sea:AddToggle("Togglelockmoon", {
    ...
    ["Description"] = "Look At Moon & Use Race",
    ["Default"] = false
...
Tabs.Item:AddSection("Kiếm (Sword)")
...
Tabs.Item:AddToggle("ToggleAutoObservationV2", {
    ...
    ["Description"] = "Observation Haki V2",
    ["Default"] = false
...
Tabs.Setting:AddSection("Cài Đặt (Settings)")
Tabs.Setting:AddToggle("ToggleAutoT", {
    ["Title"] = "Bật Tộc V3",
    ["Description"] = "Enable Race V3",
    ["Default"] = false
...
Tabs.Setting:AddToggle("ToggleAutoKen", {
    ...
    ["Description"] = "Enable Observation Haki",
    ["Default"] = false
...
-- [SetValue skipped - Library không cần]
Tabs.Setting:AddSection("Kỹ Năng Thông Thạo (Mastery Skill)")
...
Tabs.Stats:AddSection("Chỉ Số (Stats)")
Tabs.Stats:AddToggle("ToggleMelee", {
    ["Title"] = "Nâng Đấm",
    ["Description"] = "Upgrade Melee",
    ["Default"] = false
...
Tabs.Player:AddSection("Người Chơi (Player)")
...
local vu916 = Tabs.Teleport:AddSection("Thế Giới (World)")
...
Tabs.Teleport:AddSection("Đảo (Island)")
if Sea1 then
    IslandList = {
        "WindMill",
        "Marine",
...
Tabs.Visual:AddSection("Giả Mạo (Impersonation)")
...
Tabs.Fruit:AddSection("Trái Ác Quỷ (Devil Fruit)")
...
Tabs.Fruit:AddToggle("ToggleFruit", {
    ["Title"] = "Mua Trái Chọn",
    ["Description"] = "Buy Selected Fruit",
    ["Default"] = false
...
local v950 = Tabs.Fruit:AddDropdown("DropdownPermanentFruit", {
    ...
    ["Description"] = "Select Permanent Fruit",
    ["Values"] = Table_DevilFruitSniper,
    ["Multi"] = false,
    ["Default"] = 1
})
...
Tabs.Fruit:AddToggle("TogglePermanentFruit", {
    ...
    ["Description"] = "Exchange Permanent Fruit",
    ["Default"] = false
...
Tabs.Fruit:AddSection("Định Vị (Locate)")
...
Tabs.Raid:AddSection("Đột Kích (Raid)")
...
Tabs.Raid:AddToggle("ToggleNextIsland", {
    ...
    ["Description"] = "Farm Raid Mobs + Fly To Island",
    ["Default"] = false
...
Tabs.Raid:AddSection("Tắp Kích Law (Law Raid)")
Tabs.Raid:AddToggle("ToggleLaw", {
    ["Title"] = "Mua Chip Và Đánh Law",
    ["Description"] = "Buy Chip & Fight Law",
    ["Default"] = false
...
Tabs.Race:AddSection("Nâng Cấp Chủng Tộc (Race Upgrade)")
Tabs.Race:AddButton({
    ["Title"] = "Đền Thời Gian",
...
Tabs.Race:AddButton({
    ["Title"] = "Chợ Mua Gear",
    ["Description"] = "Gear Shop",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875))
        Tween2(CFrame.new(28981.552734375, 14888.4267578125, - 120.245849609375))
    end
})
Tabs.Race:AddSection("Tộc (Race)")
Tabs.Race:AddButton({
    ["Title"] = "Cửa Tộc",
    ["Description"] = "Race Gate",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875))
...
Tabs.Race:AddSection("Huấn Luyận (Training)")
local v1059 = Tabs.Race:AddToggle("ToggleFarmRace", {
    ["Title"] = "Cày Luyện Tộc",
    ["Description"] = "Farm Race Training",
    ["Default"] = false
})
local vu1060 = false
...
-- [SetValue skipped - Library không cần]
Tabs.Shop:AddSection("Khả Năng (Ability)")
Tabs.Shop:AddButton({
    ["Title"] = "Geppo",
    ["Callback"] = function()
...
Tabs.Shop:AddSection("Kiếm (Sword)")
Tabs.Shop:AddButton({
    ["Title"] = "Cutlass",
    ["Callback"] = function()
...
Tabs.Shop:AddSection("Phụ Kiện (Accessory)")
Tabs.Shop:AddButton({
    ["Title"] = "Black Cape",
    ["Callback"] = function()
...
Tabs.Shop:AddSection("Võ (Martial Arts)")
Tabs.Shop:AddButton({
    ["Title"] = "Black Leg",
    ["Callback"] = function()
...
Tabs.Shop:AddSection("Khác (Misc)")
Tabs.Shop:AddButton({
    ["Title"] = "Đổi Chỉ Sổ",
...
Tabs.Shop:AddButton({
    ["Title"] = "Đổi Tộc Ghoul",
    ["Description"] = "Change Race To Ghoul",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack({
            "Ectoplasm",
            "Change",
...
Tabs.Shop:AddButton({
    ...
    ["Description"] = "Change Race To Cyborg",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack({
            "CyborgTrainer",
            "Buy"
...
Tabs.Misc:AddSection("Khác (Misc)")
Tabs.Misc:AddButton({
...
Tabs.Misc:AddSection("Sương (Fog)")
...
local v1099 = Tabs.Sea:AddToggle("ToggleTPFrozenDimension", {
    ...
    ["Description"] = "Fly To Leviathan Island",
    ["Default"] = false
})
...
if Sea3 then
    ...
    ["Description"] = "Leviathan Chip Status",
    })
    spawn(function()
		-- upvalues: (ref) vu1102
        pcall(function()
...
Tabs.Sea:AddToggle("ToggleBlazeEmberFarm", {
    ...
    ["Description"] = "Attack Hydra Enforcer + Venomous Assailant",
    ["Default"] = false
...
Tabs.Sea:AddButton({
    ...
    ["Description"] = "Fly To Dragon Dojo Area",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance", Vector3.new(5661.5322265625, 1013.0907592773438, - 334.9649963378906))
        Tween2(CFrame.new(5814.42724609375, 1208.3267822265625, 884.5785522460938))
    end
...
Tabs.Sea:AddButton({
    ...
    ["Description"] = "Craft Volcanic Magnet | Cần 15 Lứa Đố+10 Sằt",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack({
            "CraftItem",
            "Craft",
...
Tabs.Sea:AddButton({
    ...
    ["Description"] = "Craft Dino Hood | Cần 25 Xương Kh\225\187\167ng Long + 10 Sừng Tê Giác",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack({
            "CraftItem",
            "Craft",
...
Tabs.Sea:AddButton({
    ...
    ["Description"] = "Craft T-Rex Skull | Cần 8 Xương Kh\225\187\167ng Long + 5 Vảy Rỗng",
    ["Callback"] = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack({
            "CraftItem",
            "Craft",
...
Tabs.Sea:AddToggle("ToggleCollectFireFlowers", {
    ["Title"] = "Làm Hoa Đố",
    ["Description"] = "Craft Flower Puzzle",
    ["Default"] = false
...
Tabs.Sea:AddToggle("ToggleTrialTeleport", {
    ...
    ["Description"] = "Fly To Draco Trial Gate",
    ["Default"] = false
...
Tabs.Sea:AddSection("Đảo Dung Nham (Magma Island)")
local vu1150 = Tabs.Sea:AddParagraph({
    ["Title"] = "Trạng Thái Đảo Dung Nham",
    ["Description"] = "Magma Island Status",
})
spawn(function()
	-- upvalues: (ref) vu1150
    pcall(function()
...
Tabs.Sea:AddToggle("ToggleTPVolcano", {
    ...
    ["Description"] = "Fly To Magma Island",
    ["Default"] = false
...
Tabs.Sea:AddToggle("ToggleKillAura", {
    ...
    ["Description"] = "Attack Aura Golems",
    ["Default"] = false
...
-- Ham nhat trung dung CollectionService chinh xac
local function CollectEgg()
    local char = plr.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    local hum = char and char:FindFirstChild("Humanoid")
    ...
    -- Tim Remote khong dung WaitForChild (tranh yield)
    local EggRemote = nil
    pcall(function()
        EggRemote = game:GetService("ReplicatedStorage").Modules.Net["RE/EasterServiceRE"]
    end)
    ...
    for _, egg in pairs(eggs) do
        ...
        if uid and egg:IsA("BasePart") then
            -- BAM TRUNG 10 LAN - theo duoi Eggcited Egg
            for i = 1, 10 do
            ...
            for i = 1, 5 do
                pcall(function()
                    EggRemote:FireServer("CollectEgg", {UID=uid, Additional={}})
                end)
                task.wait(0.1)
            end
            return true
...
-- Bay muot den dao bang Tween2 (giong farm mob)
local function FlyToIsland(dest)
    _G.StopTween2 = false
    pcall(function() Tween2(dest * CFrame.new(0, 8, 0)) end)
end

Tabs.Sea:AddSection("🥚 Auto Nhặt Trứng Event (🥚 Auto Collect Egg Event)")
Tabs.Sea:AddDropdown("DropdownEggMethod", {
    ["Title"] = "Chế Độ Nhặt Trứng",
    ["Description"] = "Egg Collect Mode",
    ["Values"] = {"Island", "Normal"},
    ["Default"] = "Island"
}):OnChanged(function(v) _G.ChooseEgg = v end)
...
Tabs.Sea:AddToggle("ToggleCollectEgg", {
    ...
    Library:Notify({
        Title = "LuanOri Hub",
        Content = v and "🥚 Auto Egg: BẬT" or "🥚 Auto Egg: TẮT",
        Duration = 3
    })
end)
...
spawn(function()
    ...
    while wait(1) do
        ...
        if not hasEgg and _G.ChooseEgg == "Island" then
            if #IslandPositions > 0 and (tick() - lastIslandTP) > 2 then
                isFlying = true
                local dest = IslandPositions[currentIndex]
                currentIndex = currentIndex + 1
                ...
                lastIslandTP = tick()
                Library:Notify({Title="LuanOri Hub", Content="🥚 Bay sang đảo "..currentIndex.."/"..#IslandPositions, Duration=2})
                FlyToIsland(dest)
                isFlying = false
            end
        end
...
-- ========== FARM SUMMER TOKEN ==========
-- ========== AUTO RAID PIRATE ==========
Tabs.Main:AddSection("Farm Hải Tặc Phóng Pháo (Farm Cannon Pirate)")
...
if Sea3 then
    ...
    ["Description"] = "Summon Tyrant (Summon Boss)",
        ["Default"] = false
...
-- ========== AUTO CÂU CÁ ==========
Tabs.Main:AddSection("Auto Câu Cá (Auto Fishing)")
...
-- ========== NÂNG ĐIỂM KỸ NĂNG ==========
Tabs.Setting:AddSection("Nâng Điểm Kỹ Năng (Upgrade Skill Points)")
...
Tabs.Setting:AddToggle("ToggleSPFruit", {["Title"] = "Trái Ác Quỷ",
    ["Description"] = "Devil Fruit", ["Default"] = false}):OnChanged(function(v) _spFruit = v end)
spawn(function()
...
-- ========== KỸ NĂNG ĐẶC BIỆT ==========
Tabs.Setting:AddSection("Kỹ Năng Đặc Biệt (Special Skill)")
...
-- ========== FPS BOOST ==========
Tabs.Misc:AddSection("FPS & Hiệu Suất (FPS & Performance)")
Tabs.Misc:AddButton({
    ["Title"] = "FPS Boost",
    ["Callback"] = function()
        pcall(function()
